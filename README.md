...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create gatheren-61fc
  fly apps create gatheren-61fc-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app gatheren-61fc
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app gatheren-61fc-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'gatheren-61fc-staging' does not match app name in config file 'gatheren-61fc'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name gatheren-61fc-db
  fly postgres attach --app gatheren-61fc gatheren-61fc-db

  fly postgres create --name gatheren-61fc-staging-db
  fly postgres attach --app gatheren-61fc-staging gatheren-61fc-staging-db
  ```
